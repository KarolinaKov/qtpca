...
| Proměnná | Význam |
|---|---|
| `DJANGO_SECRET_KEY` | tajný klíč Djanga |
| `ALLOWED_HOSTS` | povolené hostitele, oddělené čárkou |
| `JWT_SIGNING_KEY` | klíč pro podepisování JWT (HS256) |
| `FIELD_ENCRYPTION_KEY` | Fernet klíč pro šifrování TOTP tajemství |
| `DB_NAME`, `DB_USER`, `DB_PASSWORD`, `DB_HOST`, `DB_PORT` | připojení k PostgreSQL |
| `REDIS_URL` | broker a backend pro Celery |
| `BANK_API_URL` | URL bankovního API (aktuálně cesta k souboru, viz poznámky) |



- `data_getter.py` má stahování z bankovního API zakomentované a místo toho čte lokální soubor (`bank_module/data.txt`). Pro ostrý provoz je potřeba vrátit `requests.get` a retry logiku.
- `AuthService.verify_token` nekontroluje `token_type`, takže by endpointy měly ověřovat, že dostaly správný typ tokenu (access pro start, start pro finish).
- V `settings.py` je `DEBUG = True` a zabezpečené cookies jsou vypnuté, před nasazením je potřeba upravit.
